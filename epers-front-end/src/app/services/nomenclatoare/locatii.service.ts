import { HttpClient, HttpParams } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { BehaviorSubject, Observable, tap } from 'rxjs';
import { HeaderTableFiltersModel } from 'src/app/models/common/HeaderTableFiltersModel';
import { LocatiiDisplayModel } from 'src/app/models/nomenclatoare/LocatiiDisplayModel';
import { NLocatii } from 'src/app/models/nomenclatoare/NLocatii';
import { environment } from 'src/environments/environment';

@Injectable({
  providedIn: 'root'
})
export class LocatiiService {
  baseUrl = environment.baseUrl;

  private tableFiltersSubject: BehaviorSubject<HeaderTableFiltersModel> = new BehaviorSubject<HeaderTableFiltersModel>({
    filter: '',
    filterFirma: null,
    currentPage: 1,
    itemsPerPage: 10
  });
  private locatiiDisplayModelSubject: BehaviorSubject<LocatiiDisplayModel> = new BehaviorSubject<LocatiiDisplayModel>(null);
  tableFiltersObs$ = this.tableFiltersSubject.asObservable();
  locatiiDisplayModelObs$ = this.locatiiDisplayModelSubject.asObservable();

  constructor(private httpClient: HttpClient) { }

  public getAllPaginated(currentPage: number, itemsPerPage: number, filter: string, filterFirma: number | null) {
    const idFirmaLoggedInUser = JSON.parse(localStorage.getItem('user'))?.idFirma;
    const idRolLoggedInUser = JSON.parse(localStorage.getItem('user'))?.idRol;

    // PaginatedListRequestModel
    let params = new HttpParams();
    params = params.append("currentPage", currentPage);
    params = params.append("itemsPerPage", itemsPerPage);
    params = params.append("idRol", idRolLoggedInUser ? idRolLoggedInUser : '');
    params = params.append("idFirmaLoggedInUser", idFirmaLoggedInUser ? idFirmaLoggedInUser : '');
    params = params.append("filter", filter ? filter : '');
    params = params.append("filterFirma", filterFirma ? filterFirma : '');

    this.tableFiltersSubject.next({
      filter: filter,
      filterFirma: filterFirma,
      currentPage: currentPage,
      itemsPerPage: itemsPerPage
    });

    this.httpClient.get<LocatiiDisplayModel>(this.baseUrl + "/Locatii/paginated", { params: params }).pipe(tap((data) => {
      this.locatiiDisplayModelSubject.next(data);
    })).subscribe();
  }

  public getLocatiiData(): Observable<NLocatii[]> {
    return this.httpClient.get<NLocatii[]>(this.baseUrl + "/Locatii");
  }

  public get(id: number): Observable<NLocatii> {
    return this.httpClient.get<NLocatii>(this.baseUrl + "/Locatii/" + id);
  }

  public update(locatie: NLocatii) {
    return this.httpClient.put<NLocatii>(this.baseUrl + "/Locatii", locatie);
  }

  public add(locatie: NLocatii) {
    return this.httpClient.post(this.baseUrl + "/Locatii", locatie);
  }

  public delete(id: number) {
    return this.httpClient.delete(this.baseUrl + "/Locatii/" + id);
  }

}
